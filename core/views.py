...
def index(request):
    ...
    absent = Profile.objects.filter(present=False).order_by('classinfo','sectioninfo','first_name','last_name')
    context = {
        'scanned': scanned,
        'present': present,
        'absent': absent,
...
def profiles(request):
    profiles = Profile.objects.all().order_by('classinfo','sectioninfo','first_name','last_name')
    context = {
        'profiles': profiles
    }
...
def add_profile(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.fields['updated']= datetime.now()
            form.save()
...
def sendmailsfinal(request):
    absent = Profile.objects.filter(present=False).order_by('classinfo','first_name')
    #Add your email address in place of ***** 
    for profile in absent:
        send_mail('XYZ School Student\'s Information',
        'Dear Sir/Ma\'am, this is to inform you that your ward '+ profile.first_name + ' ' + profile.last_name+ ' is absent today.',
        '*****',
        [profile.email],
         fail_silently=False)
    return render(request,'core/sendfinal.html')
